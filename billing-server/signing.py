...
from models import User, SignRequest, new_uuid, utcnow
...
def _resolve_provider(preference: str, country_code: str) -> str:
    if preference != "auto":
        return preference
    if not country_code or country_code.upper() == "CN":
        return "tencent"
    return "deepgram"
...
def _prepare_session(
    ...
    try:
        ...
        if existing:
            if existing.settled:
                return None, user.balance_cents, False, (
            ...
            return _serialize_session(existing), user.balance_cents, False, None
        ...
        if user.balance_cents < precharge:
            return None, user.balance_cents, False, (jsonify({
        ...
        try:
            ...
            if duplicate and not duplicate.settled:
                ...
                balance = fresh_user.balance_cents if fresh_user else 0
                return _serialize_session(duplicate), balance, False, None
            return None, 0, False, (jsonify({"error": "Session conflict"}), 409)
        return _serialize_session(sign_req), user.balance_cents, True, None
    finally:
        db.close()
...
def _refund_failed_session(session_id: str, user_id: str) -> None:
    ...
    try:
        ...
        user = db.query(User).filter(User.id == user_id).with_for_update().first()
        if user:
            user.balance_cents += sign_req.precharge_cents
        sign_req.settled = 1
