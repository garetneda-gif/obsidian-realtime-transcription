...
test("panel settings expose custom AI output language and wrapped select arrow", () => {
  ...
  assert.ok(viewSource.includes('rangeWrap.addEventListener("pointermove", handleRangePointerMove)'));
  assert.ok(viewSource.includes("updateFontSizeFromClientX"));
  assert.ok(viewSource.includes('rangeWrap.addEventListener("mousedown", handleRangeMouseDown)'));
  assert.ok(viewSource.includes('window.addEventListener("mousemove", handleRangeMouseMove)'));
  assert.ok(viewSource.includes('window.addEventListener("mouseup", handleRangeMouseUp)'));
  assert.ok(viewSource.includes("new ResizeObserver(() => this.updatePanelRangeProgress(fontInput))"));
  assert.ok(viewSource.includes("private cleanupPanelSettingsPage()"));
  assert.ok(viewSource.includes("updatePanelRangeProgress(fontInput)"));
  assert.ok(viewSource.includes("window.requestAnimationFrame(() => this.updatePanelRangeProgress(fontInput))"));
  assert.ok(viewSource.includes("const thumbWidth = 26"));
  assert.ok(viewSource.includes('input.parentElement?.style.setProperty("--panel-range-progress", progress)'));
  assert.ok(viewSource.includes("input.parentElement?.clientWidth"));
  assert.ok(stylesSource.includes("--panel-range-progress"));
  assert.ok(stylesSource.includes("grid-template-columns: 56px minmax(0, 1fr)"));
  assert.ok(stylesSource.includes(".panel-settings-range-wrap"));
  assert.ok(stylesSource.includes("touch-action: none"));
  assert.ok(stylesSource.includes("pointer-events: auto"));
  assert.ok(stylesSource.includes("z-index: 3"));
  assert.ok(stylesSource.includes(".panel-settings-range-track"));
  assert.ok(stylesSource.includes(".panel-settings-range-progress"));
  assert.ok(stylesSource.includes(".panel-settings-range-thumb"));
  assert.ok(stylesSource.includes("background: #f6a313"));
  assert.ok(stylesSource.includes("border-radius: 50%"));
  ...
  assert.ok(stylesSource.includes(".transcript-card.summary-card .summary-body ul"));
  assert.ok(stylesSource.includes("margin: 4px 0 0"));
